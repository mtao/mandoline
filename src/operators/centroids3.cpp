...
namespace {
template <mtao::eigen::concepts::ColVecs2Compatible ColVecs>
mtao::Vec3d brep_centroid(const CutFace<3> &cutface, const ColVecs &V,
...
mtao::ColVecs3d cell_centroids(const CutCellMesh<3> &ccm) {
    /*
           ...
           */
    mtao::ColVecs3d face_brep_cents(3, ccm.face_size());
    ...
    mtao::ColVecs3d V(3, ccm.cell_size());
    V.setZero();
...
mtao::ColVecs3d face_centroids(const CutCellMesh<3> &ccm) {
    /*
           ...
           V.topRows(StaggeredGrid::cell_size()).array() = dx().prod();
           */
    mtao::ColVecs3d ret(3, ccm.cut_face_size());
    auto Vs = ccm.vertices();
