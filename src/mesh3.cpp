...
namespace mandoline {
    ...
    size_t CutCellMesh<3>::cell_size() const {
        return adaptive_grid.cells.size() + cells.size();
    }
    ...
    bool CutCellMesh<3>::is_mesh_face(int idx) const {
        return mesh_faces.find(idx) != mesh_faces.end();
    }
    auto CutCellMesh<3>::cell_volumes() const -> VecX{
        ...
        V.setZero();
        auto Vs = vertices();
        mtao::VecXd face_brep_vols(faces.size());
        for(auto&& [i,f]: mtao::iterator::enumerate(faces)) {
            face_brep_vols(i) = f.brep_volume(Vs);
        }

        for(auto&& [k,c]: mtao::iterator::enumerate(cells)) {
        ...
        for(auto&& [i,c]:  adaptive_grid.cells) {
    ...
    std::vector<std::set<int>> CutCellMesh<3>::cell_faces() const {
        ...
        std::transform(cells.begin(),cells.end(), ret.begin(), [](auto&& V) {
                std::set<int> ret;
...
#pragma omp parallel for
    ...
    void CutCellMesh<3>::write_obj(const std::string& prefix, const std::set<int>& indices, const std::optional<int>& region, bool show_indices, bool show_base, bool show_flaps, bool mesh_faces) const {
        std::stringstream ss;
        auto CS = StaggeredGrid::cell_shape();
        ...
        for(auto&& [F,index]: mtao::iterator::zip(Fs,indices)) {
            if(mesh_faces) {
                if(is_cut_cell(index)) {
                    ...
                    for(auto&& [fidx,s]: cells.at(index)) {
                        std::cout << fidx << " ";
                        auto&& f = faces[fidx];
                        if(f.is_mesh_face()) {
                ...
                if(is_cut_cell(index)) {
                    ...
                    if(show_base) {
                        F = adaptive_grid.triangulated(index);
                    }
                }
            }
        }
        ...
        for(auto&& [F,index]: mtao::iterator::zip(Fs,indices)) {
            if(is_cut_cell(index)) {
                ofs << "# cell" << index << ": ";
                for(auto&& [c,b]: cells.at(index)) {
                    ofs << c << " ";
                }
            } else {
                ofs << "# adaptive cube" << index << ": ";
                auto&& c = adaptive_grid.cells.at(index);
    ...
    std::vector<bool> CutCellMesh<3>::boundary_faces() const {
        std::vector<bool> ret(faces.size(),false);
        for(auto&& [m,_]: mesh_faces) {
            ret[m] = true;
        }
        return ret;
    }
    ...
    std::vector<int> CutCellMesh<3>::regions(bool boundary_sign_regions) const {
        ...
        if(boundary_sign_regions) {
            ...
            for(auto&& [idx,c]: mtao::iterator::enumerate(cells)) {
                Edge counts{{0,0}};// 1 -1 
                for(auto&& [f,b]: c) {
                    std::cout << f << "/" << mesh_faces.size() << std::endl;
                    if(is_mesh_face(f)) {
                        counts[b?0:1]++;
                    }
                }
            ...
            for(int i = 0; i < cells.size(); ++i) {
                ret[i] = cells[i].region;
            }
            for(auto&& [c,r]: adaptive_grid_regions) {
                ret[c] = r;
            }
        }
        return ret;
    ...
    void  CutCellMesh<3>::serialize(CutMeshProto& cmp) const {
        ...
        for(int i = 0; i < origV.cols(); ++i) {
            protobuf::serialize(origV.col(i),*cmp.add_origv());
        }
        for(int i = 0; i < origF.cols(); ++i) {
            protobuf::serialize(origF.col(i),*cmp.add_origf());
        }
        ...
        }
        {
            ...
            for(auto&& [c,cell]: adaptive_grid.cells) {
                cell.serialize(cmap[c]);
            }
            auto& rmap = *cmp.mutable_cube_regions();
            for(auto&& [a,b]: adaptive_grid_regions) {
                rmap[a] = b;
            }
        }
    }
    ...
    CutCellMesh<3> CutCellMesh<3>::from_proto(const CutMeshProto& cmp) {
        ...
        ret.origV.resize(3,cmp.origv().size());
        for(int i = 0; i < ret.origV.cols(); ++i) {
            ret.origV.col(i) = protobuf::deserialize(cmp.origv(i));
        }
        ret.origF.resize(3,cmp.origf().size());
        for(int i = 0; i < ret.origF.cols(); ++i) {
            ret.origF.col(i) = protobuf::deserialize(cmp.origf(i));
        }
        ret.faces.resize(cmp.faces().size());
        for(int i = 0; i < cmp.faces().size(); ++i) {
            ret.faces[i] = CutFace<3>::from_proto(cmp.faces(i));
        }
        ret.cells.resize(cmp.cells().size());
        for(int i = 0; i < cmp.cells().size(); ++i) {
            ret.cells[i] = CutCell::from_proto(cmp.cells(i));
        }
        std::copy(cmp.foldedfaces().begin(),cmp.foldedfaces().end(),std::inserter(ret.folded_faces,ret.folded_faces.end()));
        ...
        for(auto&& [idx,btf]: cmp.mesh_faces()) {
            ...
            ret.mesh_faces[idx] = {B,pid};
        }

        for(auto&& [a,b]: cmp.cubes()) {
            ret.adaptive_grid.cells[a] = AdaptiveGrid::Cell::from_proto(b);
        }
        for(auto&& [a,b]: cmp.cube_regions()) {
            ret.adaptive_grid_regions[a] = b;
        }
    ...
    mtao::ColVecs3i CutCellMesh<3>::triangulate_face(int face_index) const {
        ...
        return faces[face_index].triangulate(subVs);
    }
...
#pragma omp parallel for
    ...
    auto CutCellMesh<3>::face_volumes(bool from_triangulation) const -> VecX{
        ...
        if(from_triangulation) {
            ...
            auto trimesh_vols = mtao::geometry::volumes(origV,origF);
    ...
    Eigen::SparseMatrix<double> CutCellMesh<3>::barycentric_matrix() const {
        Eigen::SparseMatrix<double> A(vertex_size(), origV.cols());
        std::vector<Eigen::Triplet<double>> trips;
        std::map<std::array<int,2>,double> mp;
        for(auto&& [fid, btf]: mesh_faces) {
            auto t = btf.sparse_entries(faces[fid], origF);
    ...
    Eigen::SparseMatrix<double> CutCellMesh<3>::face_barycentric_volume_matrix() const {
        ...
        if(origF.size() == 0) {
            ...
            face_size = origF.cols();
        }
        Eigen::SparseMatrix<double> A(faces.size(),face_size);
        std::vector<Eigen::Triplet<double>> trips;
        for(auto&& [fid, btf]: mesh_faces) {
    ...
    Eigen::SparseMatrix<double> CutCellMesh<3>::boundary() const {
        auto trips = adaptive_grid.boundary_triplets(faces.size());
        return {};
        int num_faces = 0;
        int num_cells = 0;
        for(auto&& t: trips) {
    ...
    mtao::ColVecs3i CutCellMesh<3>::triangulated_cell(int idx, bool use_base, bool use_flap) const {
        if(is_cut_cell(idx)) {
            ...
            for(auto&& [fidx,b]: cells[idx]) {
                bool is_flap = folded_faces.find(fidx) != folded_faces.end();
                bool is_base = !is_flap;
                if((use_flap && is_flap) || (use_base && !is_flap)) {
                    auto& tri = faces[fidx].triangulation;
            ...
            if(use_base) {
                return adaptive_grid.triangulated(idx);
            }
        }
        return {};
    }
