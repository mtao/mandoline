...
bool CutCellGenerator<3>::AxisHEMData::is_boundary_cell(
...
void CutCellGenerator<3>::bake_faces() {
    ...
    {
        auto t = mtao::logging::profiler("caching axial primal faces", false,
                                         "profiler");
    ...
    std::array<CutCellEdgeGenerator<2>, D> grids{
        {subgrid(*this, 0), subgrid(*this, 1), subgrid(*this, 2)}};
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        auto &&g = grids[i];
        // for(auto&& [dim,ahdata,axialEdges_dim,g_]:
        // mtao::iterator::enumerate(axis_hem_data, axialEdges,grids)) { auto& g
        // = g_;
...

        {
            ...
            std::transform(axialEdges_dim.begin(), axialEdges_dim.end(),
            ...
            for (it = axial_edge_indices.begin(); it < axial_edge_indices.end();
                ...
                int cidx = *it;
                    ...
                    std::transform(E.begin(), E.end(),
                                   std::inserter(ahd.edges, ahd.edges.end()),
                                   [](auto s) {
                                       if (s[0] > s[1]) {
                                           std::swap(s[0], s[1]);
                                       }
                    ...
                    // std::cout << "AHD size: " << ahd.edges.size() <<
                    // std::endl;
                    ...
                    //    std::endl;
                    //}
                    std::transform(bedges.begin(), bedges.end(),
                                   std::inserter(ahd.boundary_edges,
                                                 ahd.boundary_edges.end()),
                                   [&, dim = dim](Edge e) -> Edge {
                                       for (auto &&idx : e) {
...
void CutCellGenerator<3>::compute_faces() {
    ...
    // the sign must be determined by the cell it shares, we just write to
    // external_boundary and let generate_celll fill in the signs
            ...
            auto pc = possible_cells(I);
            {
                ...
                if (pca[0][idx] < 0) {
                    ...
                    f.external_boundary = {-2, 0};
                           ...
                           I.begin()->size() ==
                               4) {  // should always be a grid cell!
...
void CutCellGenerator<3>::compute_faces_vertex() {
    ...
    for (auto &&[i, f] : mtao::iterator::enumerate(m_cut_faces)) {
        if (f.indices.size() > 2) {  // && f.mask().count() < 2) {
            ...
            // std::cout << "Normal dots: " << f.parent_fid << ": " <<
            // origN.col(f.parent_fid).dot(data().m_triangle_intersections[f.parent_fid].N())
            // << std::endl;
...
mtao::map<int, CutFace<3>> CutCellGenerator<3>::compute_faces_axis(
    int idx, int cidx) const {
