...
const AdaptiveGridFactory::Indexer AdaptiveGridFactory::cmask_indexer(
    std::array<int, 3>{{width, width, width}});
const AdaptiveGridFactory::Indexer AdaptiveGridFactory::vmask_indexer(
    std::array<int, 3>{{1 + width, 1 + width, 1 + width}});
...
const std::array<AdaptiveGridFactory::Indexer, 3>
    AdaptiveGridFactory::mask_edge_indexers = make_edge_indexers();
const std::array<AdaptiveGridFactory::coord_type, 3>
    AdaptiveGridFactory::mask_edge_shapes = make_mask_edge_shapes();
...
auto AdaptiveGridFactory::compute_axial_edges(
    ...
    std::array<coord_type, 3> edge_shapes{
    ...
    for (auto &&[level, g] : mtao::iterator::enumerate(levels)) {
        ...
        if (max_level && *max_level == level) {
            if (level == 0) {
                ...
                for (auto &&[A, B, C, D] :
                     mtao::iterator::zip(edges, Es, bedges, BEs)) {
                ...
                GridData3 A(g.shape());
                std::transform(
                    g.begin(), g.end(), A.begin(),
                    [&](const ActiveMask &mask) -> bool { return mask.any(); });
...
void AdaptiveGridFactory::set_edge_masks(const coord_type &shape,
                                         const coord_type &corner,
                                         std::array<GridData3, 3> &edge_masks,
                                         const GridAccessor &accessor) const {
...
auto AdaptiveGridFactory::compute_edges(const std::optional<int> &max_level)
    const -> std::tuple<std::set<Edge>, AxialBEdgeMap> {
    std::set<Edge> edges;
...
auto AdaptiveGridFactory::get_edges(const std::array<GridData3, 3> &edge_masks,
    ...
    for (auto &&[dim, g, og] :
         mtao::iterator::enumerate(edge_masks, eorigmasks)) {
        int bidx0 = (dim + 1) % 3;
        int bidx1 = (dim + 2) % 3;
...
auto AdaptiveGridFactory::get_edge(const coord_type &start, int jump,
                                   int dim) const -> Edge {
    coord_type end = start;
    end[dim] += jump;
    return Edge{{vertex_index(start), vertex_index(end)}};
}
auto AdaptiveGridFactory::make_edge_shapes(const coord_type &coord) const
    -> std::array<coord_type, 3> {
...
auto AdaptiveGridFactory::make_edge_mask(const GridData3b &mask,
                                         int level) const
    -> std::array<GridData3, 3> {
        ...
        for (int b = 0; b < mask.shape()[1]; ++b) {
            for (int c = 0; c < mask.shape()[2]; ++c) {
                ...
                if (m.any()) {
                    ...
                    for (int k = 0; k < 3; ++k) {
                        for (int a = abc[0];
                             a < (abc[0] + width + ((k == 0) ? 0 : 1)); ++a) {
                            for (int b = abc[1];
                                 b < (abc[1] + width + ((k == 1) ? 0 : 1));
                                 ++b) {
                                for (int c = abc[2];
                                     c < (abc[2] + width + ((k == 2) ? 0 : 1));
                                     ++c) {
                    ...
                    set_edge_masks(cmask_indexer.shape(), abc, edge_masks,
                                   [&](const coord_type &abc) {
...
auto AdaptiveGridFactory::make_edge_mask(const GridData3 &mask, int level) const
    -> std::array<GridData3, 3> {
    auto edge_masks = empty_edge_masks(mask.shape(), true);

    set_edge_masks(mask.shape(), coord_type{{0, 0, 0}}, edge_masks,
                   [&mask](const coord_type &abc) { return mask(abc); });
...
auto AdaptiveGridFactory::make_edges(const GridData3b &mask, int level) const
    -> std::tuple<std::array<std::set<Edge>, 3>, AxialBEdgeMap> {
    return get_edges(make_edge_mask(mask, level), level);
}
auto AdaptiveGridFactory::make_edges(const GridData3 &mask, int level) const
    -> std::tuple<std::array<std::set<Edge>, 3>, AxialBEdgeMap> {
    return get_edges(make_edge_mask(mask, level), level);
}

AdaptiveGridFactory::AdaptiveGridFactory(const GridData3 &mask)
    : original(!mask) {
    ...
    spdlog::trace("Adaptive grid factory level count: {} with size: {}",
                  level_count, size);
    levels.resize(level_count);
    levels_mask.resize(level_count);
    levels[0] =
        GridData3b::Constant(0, size / width, size / width, size / width);
                ...
                for (int ii = 0; ii < width; ++ii) {
                    ...
                    for (int jj = 0; jj < width; ++jj) {
                        ...
                        for (int kk = 0; kk < width; ++kk) {
                            ...
                            levels[0](i, j, k)[cmask_index(ii, jj, kk)] =
                                original(I, J, K);
                        }
                    }
                }
                    ...
                    for (int a = 0; a < width; ++a) {
                        ...
                        for (int b = 0; b < width; ++b) {
                            ...
                            for (int c = 0; c < width; ++c) {
                                int ccc = width * cc + c;
                                next_mask(aaa, bbb, ccc) =
                                    v[cmask_indexer.index(a, b, c)] =
                                        me(aaa, bbb, ccc).any();
                            }
                        }
                    }
    ...
    for (auto &&[level, g] : mtao::iterator::enumerate(levels)) {
        if (max_level && *max_level == level) {
            if (level == 0) {
                ...
                auto &g = levels[level - 1];
                GridData3 A(g.shape());
                std::transform(g.begin(), g.end(), A.begin(),
                               [&](const ActiveMask &mask) -> bool {
                                   return !mask.none();
                               });
                make_cells(A, level);
                break;
...
auto AdaptiveGridFactory::grid_from_cells(
    const std::map<int, Cell> &cells) const -> GridData3i {
...
void AdaptiveGridFactory::make_cells(const ActiveMask &mask, int level,
