...
class ExteriorGrid : public mtao::geometry::grid::StaggeredGrid<double, D>,
   ...
   public:
    ...
    int num_boundary_facets() const {
        return DomainBoundary::boundary_facet_size();
    }
    ...
    const std::vector<int> &boundary_facet_axes() const {
        return m_boundary_facet_axes;
    }
    int boundary_facet_axis(size_t idx) const {
        return m_boundary_facet_axes.at(idx);
    }
    const coord_type &cell_coord(size_t idx) const {
        return m_cell_coords.at(idx);
    }
   ...
   private:
    GridDatai m_cell_indices;  // acts as a hash map for cell indexing based off
                               // a grid
    std::vector<coord_type> m_cell_coords;  // reporst the cell found in each
                                            // index (inverts m_cell_indices)
    std::vector<int>
        m_boundary_facet_axes;   // parallel with
                                 // DomainBoundary::boundary_facet_pairs
